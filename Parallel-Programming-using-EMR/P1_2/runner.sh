...
answer_2() {
	# Write a function to get the answer to Q2. Do not just echo the answer.
       sort -t $'\t' -k1,1n -k2,2r  < output | head -n1 | awk  {'print $2,"\t",$1'} 
}
...
answer_3() {
        ...
        sed -e "s/20151218://" output | sort -t $'\t' -k 20rn |head -1 | awk '{print $2"\t"$20}'
}
...
answer_4() {
        ...
        sed -e "s/20151201://" output | sort -t $'\t' -k 3n | awk '{if($3==0)print;}'| sort -t $'\t' -k 1rn | head -1 |  awk  {'print $2'}
}
...
answer_6() {
	...
	#done < q6
        javac Q6.java 
        java -cp . Q6 > tempq6.txt
        sort -t $'\t'  -k2nr  < tempq6.txt | awk -v ORS=, '{ print $1 }' | sed 's/,$//'
}
...
answer_7() {
    ...
    #done < q7
       javac Q7.java
       java -cp . Q7 > tempq7.txt
       sort  -k2,2nr -k1,1r < tempq7.txt | awk -v ORS=, '{ print $1 }' | sed 's/,$//'      
}
...
answer_11() {
    # echo the answer (instance type)
    echo "5 core instance of m3.xlarge(Spot:0.05)"
}
...
answer_13() {
	...
	echo "EMR took 1 hour 18 minutes i.e 78 minutes"
}
